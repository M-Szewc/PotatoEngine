...
void swapchain_destroy(vulkan_context* context, vulkan_swapchain* swapchain);
...
void vulkan_swapchain_create(
    ...
    swapchain_create(context, width, height, out_swapchain);
}
...
void vulkan_swapchain_recreate(
    ...
    swapchain_create(context, width, height, swapchain);
}
...
void vulkan_swapchain_present(
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR) {
        ...
        PE_FATAL("Failed to present swapchain image!");
    }
...
void swapchain_create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain){
    ...
    swapchain_extent.width = PE_CLAMP(swapchain_extent.width, min.width, max.width);
    swapchain_extent.height = PE_CLAMP(swapchain_extent.height, min.height, max.height);

    u32 image_cout = context->device.swapchain_support.capabilities.minImageCount + 1;
    ...
    swapchain_create_info.minImageCount = image_cout;
    ...
    if (!swapchain->images) {
        swapchain->images = (VkImage*)pe_allocate(sizeof(VkImage) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    if (!swapchain->views) {
        swapchain->views = (VkImageView*)pe_allocate(sizeof(VkImageView) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    ...
    if (!vulkan_device_detect_depth_format(&context->device)) {
        ...
        PE_FATAL("Failed to find a supported format!");
    }
    ...
    PE_INFO("Swapchain created succesfully!");

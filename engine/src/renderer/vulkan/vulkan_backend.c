...
b8 vulkan_renderer_backend_initialize(renderer_backend* backend, const char* application_name, struct platform_state* plat_state) {
    ...
    if (!vulkan_object_shader_create(&context, &context.object_shader)) {
        PE_ERROR("Error loading built-in basic_lighting shader.");
        return false;
    }
    ...
    pe_zero_memory(verts, sizeof(vertex_3d) * vert_count);
    ...
    u32 indices[index_count] = {0, 1, 2, 3, 1, 0};
...
b8 create_buffers(vulkan_context* context) {
    ...
    if (!vulkan_buffer_create(
        ...
        PE_ERROR("Error creating vertex buffer.");
        return false;
    }
    context->geometry_vertex_offset = 0;

    const u64 index_buffer_size = sizeof(u32) * 1024 * 1042;
    if (!vulkan_buffer_create(
        context,
        index_buffer_size,
        ...
        PE_ERROR("Error creating index buffer.");
        return false;
    }

...
b8 vulkan_pipeline_create(
    ...
    pe_zero_memory(&color_blend_attachment_state, sizeof(VkPipelineColorBlendAttachmentState));
    ...
    binding_description.binding = 0; // Ninding index
    ...
    if (vulkan_result_is_success(result)) {
        PE_DEBUG("Graphics pipeline created!");
        return true;
    }

    PE_ERROR("vkCreateGraphicsPipelines failed with '%s'.", vulkan_result_string(result, true));
    return false;
}
